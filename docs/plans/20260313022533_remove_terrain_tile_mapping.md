...
- [x] `supabase/migrations/20260106000000_create_tiles.sql`에서 `terrains_tiles` 테이블 정의를 직접 제거하고, 이를 대체하던 새 마이그레이션 파일을 저장소에서 삭제한다.
- [x] `terrains_tiles` 제거 결과가 `src/lib/types/supabase.ts`와 `src/lib/types/supabase.generated.ts`에 반영되도록 타입 생성 산출물을 기존 마이그레이션 기준으로 다시 정리한다.
...
- [x] PR #31 기준으로 지형-타일 제거 작업이 실제로 모두 완료됐는지 코드 기준으로 재검증하고, 누락이 있으면 같은 브랜치에서 보완한다.

### 테스트 엔지니어

- [x] 지형-타일 매핑 제거와 관련된 테스트를 추가 또는 갱신해 제거된 타입/라우트/스토어 참조 회귀를 검증한다.
- [x] 새 `terrains_tiles` 삭제용 마이그레이션 파일이 남아 있지 않고, 기존 마이그레이션 수정 기준으로 검증이 수행됐는지 확인한다.
- [x] 플랜의 체크리스트 상태와 실제 변경 산출물을 대조하고 필요한 수동 검증 결과를 기록한다.
- [x] 기존 마이그레이션 수정 반영 후 테스트/검증 범위를 다시 실행하고, 남은 이슈 또는 완료 판단 근거를 PR과 플랜에 기록한다.
...
- `[플랫폼 엔지니어, 테스트 엔지니어]`
- `20260313022533_remove_terrain_tile_mapping 플랜 구현 시작`
- `supabase/migrations/20260106000000_create_tiles.sql`에서 `terrains_tiles` 테이블 정의와 정책 블록을 제거했고, `supabase/migrations/20260313030000_drop_terrains_tiles.sql`는 삭제했다.
- 검증 재실행 기준은 기존 회귀 테스트(`src/lib/hooks/use-terrain.spec.ts`, `src/lib/hooks/use-scenario.spec.ts`)와 타입 체크(`pnpm check`)이며, 저장소 검색으로 앱 타입/런타임 코드에 `terrains_tiles` 참조가 남지 않았음을 다시 확인한다.
- PR #31 기준 재검증 결과, 저장소 내 `terrains_tiles` 잔여 참조는 플랜 문서와 회귀 테스트 설명 문자열만 남아 있으며 앱 코드와 마이그레이션 산출물에는 더 이상 남지 않는다.
