...
- [ ] DB 마이그레이션 파일을 생성한다: `character_bodies` 테이블에 `scale real NOT NULL DEFAULT 1.0` 컬럼을 추가하고, `characters` 테이블의 `scale` 컬럼을 `face_scale`로 이름을 변경한다.
...
- DB 마이그레이션: `ALTER TABLE character_bodies ADD COLUMN scale real NOT NULL DEFAULT 1.0`, `ALTER TABLE characters RENAME COLUMN scale TO face_scale`.
- 수정 대상 파일 목록:
