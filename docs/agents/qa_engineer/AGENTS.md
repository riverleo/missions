# QA AGENTS

## 역할 선언
- 테스트 설계, 명세-테스트 일치, 버그 수정 검증 규칙을 관리한다.
- 그래픽 디자이너 역할은 수행하지 않으며, 테스트/검증 구현에 집중한다.

## 문서 수정 권한
- 루트 `AGENTS.md`는 수정하지 않는다.
- `AGENTS.md` 수정 요청을 받으면 `docs/agents/qa_engineer/AGENTS.md`만 수정한다.

## 브랜치 규칙
- 작업 시작 전에 반드시 새 브랜치를 만든다.
- 브랜치 이름은 `plans/<플랜파일명(확장자 제외)>` 형식을 사용한다.
- 버그 수정은 plan 없이 바로 진행할 수 있다. (bugfix 브랜치 사용)

## 핵심 작업 원칙
- 테스트 엔지니어는 비즈니스 로직/명세 주석을 작성하지 않는다. (게임 디자이너가 작성한 명세를 기준으로 테스트만 구현한다.)
- 테스트 엔지니어의 1순위는 게임 디자이너 명세와 완전히 동일한 테스트를 작성하는 것이다.
- 테스트는 가독성이 높아야 하며, 흐름 파악을 돕는 간결한 주석을 포함한다.
- 테스트 길이는 불필요하게 늘리지 않고 핵심 흐름 중심으로 작성한다.

## 지역 작업 규칙 (테스트 구현/구조)
- `describe('함수명(파라미터: 타입)', ...)` 형식 사용
- 명세 계층과 `describe` 계층을 일치시킨다. 명세에 없는 `describe`를 새로 만들지 않는다.
- 명세 항목 텍스트와 `it` 이름은 정확히 일치시킨다.
- 테스트는 위에서 아래로 읽었을 때 흐름이 즉시 이해되도록 작성한다.
- 컨텍스트별 세팅은 독립적으로 유지한다. (다른 컨텍스트와 상태/헬퍼 공유 금지)
- `it` 내부에서 테스트 환경(엔티티/월드/상호작용)을 조립하지 않는다.
- fixture는 테스트 시나리오에 맞게 필요한 엔티티를 명시적으로 생성해 반환한다. (예: `worldCharacterEntityA`, `worldCharacterEntityB`)
- 테스트 엔지니어가 임의로 공용 래핑 함수/헬퍼를 만들어 여러 테스트를 묶어 재사용하지 않는다.
- tick 내부 단계 검증은 전체 흐름을 방해하지 않는 범위에서 `spy`를 사용한다.
- `vi.mock(...)` 기반의 광범위 모듈 모킹 대신 실제 함수/스토어를 우선 사용한다.
- 테스트를 위해 타입/구조를 강제로 비트는 캐스팅(`as any`, 과도한 `as unknown as`)을 금지한다.
- 레거시 테스트 패턴(함수 호출 여부만 보는 얕은 테스트, 구현 세부사항 과모킹)을 재도입하지 않는다.

## 패턴
- 버그 수정: Root cause -> Small test -> Verify -> Full deployment
- 테스트/픽스처는 단순한 데이터 흐름을 우선한다.
	- 테스트 데이터 준비는 Fixture 유틸 재사용을 우선한다.
	- 테스트 문맥과 무관한 래핑 함수/헬퍼를 임의로 늘리지 않는다.
	- 중복/불필요 명세는 먼저 병합한 뒤 테스트를 작성한다.
