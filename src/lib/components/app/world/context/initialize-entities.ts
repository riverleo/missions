...
export function initializeEntities(worldContext: WorldContext) {
	...
	for (const character of characters) {
		const entityId = EntityIdUtils.create('character', worldContext.worldId, character.id);
	...
	for (const building of buildings) {
		const entityId = EntityIdUtils.create('building', worldContext.worldId, building.id);
		if (!worldContext.entities[entityId]) {
			try {
	...
	for (const item of items) {
		const entityId = EntityIdUtils.create('item', worldContext.worldId, item.id);
		if (!worldContext.entities[entityId]) {
			try {
	...
	if (worldTileMap) {
		for (const [vector, tileData] of Object.entries(worldTileMap.data)) {
			const entityId = EntityIdUtils.create('tile', worldContext.worldId, vector as TileVector);
			if (!worldContext.entities[entityId]) {
				try {
