...
export default function checkActionCompletion(
	...
	if (behaviorAction.type === 'once') {
		...
		if (!worldCharacterEntity.behavior.targetEntityId) {
			return !worldCharacterEntity.behavior.interactionTargetId;
		}
		...
		if (distance >= TARGET_ARRIVAL_DISTANCE) return false; // 아직 도착하지 않음
	...
	if (behaviorAction.type === 'idle') {
		return (
			tick - (worldCharacterEntity.behavior.behaviorTargetStartTick ?? 0) >=
