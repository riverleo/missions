 ...
 * 5. tickActionSystemItemPick: item_pick 시스템 액션 실행/완료 판정
 * 6. tickNextOrClear: 다음 행동으로 전환 또는 종료
 */
export default function tick(this: WorldCharacterEntityBehavior, tick: number): void {
	...
	if (this.tickActionSystemItemPick(tick)) return; // 5. item_pick 시스템 액션 실행
	this.tickNextOrClear(tick); // 6. 다음 행동 전환/종료
}
