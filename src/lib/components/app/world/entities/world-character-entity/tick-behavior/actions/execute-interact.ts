	...
	if (interaction) {
		...
		if (interactionActions.length > 0) {
			...
			if (!worldCharacterEntity.currentInteractionTargetId) {
				...
				worldCharacterEntity.currentInteractionTargetId = InteractionIdUtils.create(firstAction);
				worldCharacterEntity.interactionTargetStartTick = tick;
			...
			if (currentAction && currentAction.duration_ticks) {
				const elapsed = tick - worldCharacterEntity.interactionTargetStartTick;
	...
	if (isNeedAction && behaviorAction.need_fulfillment_id) {
		...
		if (fulfillment && fulfillment.increase_per_tick) {
			...
			if (currentNeed) {
				...
				worldCharacterEntity.worldCharacterNeeds[needId] = {
					...currentNeed,
					value: newValue,
				};
			}
