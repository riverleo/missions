...
import type createForTickAction from '$lib/hooks/fixture/world-character-entity/create-for-tick-action-once-building-use';

describe('tickActionOnceBuildingUse(tick: number)', () => {
	let fixture: ReturnType<typeof createForTickAction>;
	let entity: WorldCharacterEntity;

	beforeEach(() => {
		fixture = Fixture.worldCharacterEntity.createForTickActionOnceBuildingUse();
	...
	it('behavior.currentInteractionTargetId를 기준으로 자기가 실행할 액션이 아닌 경우 스킵한다.', () => {
		...
		const result = entity.behavior.tickActionOnceBuildingUse(0);
