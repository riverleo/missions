<script lang="ts">
	...
	interface Props {
		action: ItemBehaviorAction | undefined;
		hasParent?: boolean;
	}
	...
	const currentBehavior = $derived(
		changes?.behavior_id ? $itemBehaviorStore.data[changes.behavior_id] : undefined
	);
	...
	async function onsubmit(e: SubmitEvent) {
		...
		try {
			...
			if (changes.root) {
				const allActions = Object.values($itemBehaviorActionStore.data);
				const otherRootActions = allActions.filter(
					(a) => a.behavior_id === behaviorId && a.id !== actionId && a.root
				);
				await Promise.all(
					otherRootActions.map((a) => admin.updateItemBehaviorAction(a.id, { root: false }))
				);
			}
			...
			// 선택 해제
			const nodeId = `item-behavior-action-${actionId}`;
	...
	function onclickCancel() {
		...
		flowNodes.update((ns) =>
			ns.map((n) =>
				n.id === createItemBehaviorActionNodeId(action) ? { ...n, selected: false } : n
			)
		);
	}
</script>
			...
			{#if changes}
				<form {onsubmit} class="space-y-4">
					<div class="space-y-2">
						...
						{#if previewItemState && previewBodyState}
							<Separator />
