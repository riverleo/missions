<script lang="ts">
	...
	import ItemBehaviorCommand from './item-behavior-command.svelte';
	import ItemBehaviorCreateDialog from './item-behavior-create-dialog.svelte';
	import ItemBehaviorUpdateDialog from './item-behavior-update-dialog.svelte';
	import ItemBehaviorDeleteDialog from './item-behavior-delete-dialog.svelte';
		...
		<ButtonGroup>
			...
			<ButtonGroup>
				<Tooltip>
					...
					<TooltipContent>새로운 아이템 행동 생성</TooltipContent>
				</Tooltip>
				<Tooltip>
					<TooltipTrigger>
						{#snippet child({ props })}
							<Button
								...
								onclick={() =>
									currentBehaviorId && openDialog({ type: 'update', behaviorId: currentBehaviorId as ItemBehaviorId })}
							>
								<IconEditCircle class="size-4" />
							</Button>
						{/snippet}
					</TooltipTrigger>
					<TooltipContent>아이템 행동 수정</TooltipContent>
				</Tooltip>
			</ButtonGroup>
		</ButtonGroup>
		<ButtonGroup>
				...
				<TooltipTrigger>
					{#snippet child({ props })}
						<Button
							...
							disabled={!currentBehaviorId}
							onclick={() =>
								currentBehaviorId && openDialog({ type: 'delete', behaviorId: currentBehaviorId as ItemBehaviorId })}
						>
							<IconTrash class="size-4" />
						</Button>
					{/snippet}
				</TooltipTrigger>
				<TooltipContent>아이템 행동 삭제</TooltipContent>
			</Tooltip>
		</ButtonGroup>
	</ButtonGroup>
...
<ItemBehaviorCreateDialog />
<ItemBehaviorUpdateDialog />
<ItemBehaviorDeleteDialog />
