<script lang="ts">
	...
	import {
		...
		isItemInteractionActionNextEdgeId,
	...
	const selectedAction = $derived(
		selectedNode?.type === 'action'
			? actions.find((a) => a.id === parseItemInteractionActionNodeId(selectedNode.id))
			: undefined
	);
	const selectedActionHasParent = $derived(
		selectedAction
			? actions.some((a) => a.next_item_interaction_action_id === selectedAction.id)
			: false
	);

	const nodeTypes = {
		action: ItemInteractionActionNode,
	};
	...
	function isValidConnection(connection: Connection | Edge) {
		...
		if (sourceNode.type === 'action' && targetNode.type === 'action') {
			...
			const sourceActionId = parseItemInteractionActionNodeId(connection.source);
			const sourceAction = actions.find((a) => a.id === sourceActionId);
			if (sourceAction && sourceAction.next_item_interaction_action_id) {
				return false;
			}
	...
	const onconnectend: OnConnectEnd = async (event, connectionState) => {
		...
		const fromActionId = parseItemInteractionActionNodeId(sourceNode.id);
		...
		if (fromHandleId === 'next' && fromAction.next_item_interaction_action_id) return;
		...
		try {
			...
			if (newAction) {
				nodes = nodes.map((n) =>
					n.id === createItemInteractionActionNodeId(newAction) ? { ...n, position } : n
				);
			}
		} catch (error) {
			skipConvertEffect = false;
	...
	function convertToNodesAndEdges() {
		...
		actions.forEach((action, index) => {
			...
			// 이 액션을 가리키는 부모 액션 찾기
			const parentAction = actions.find((a) => a.next_item_interaction_action_id === action.id);

			const id = createItemInteractionActionNodeId(action);
			newNodes.push({
				id,
				type: 'action',
		...
		actions.forEach((action) => {
			if (action.next_item_interaction_action_id) {
				const targetAction = actions.find((a) => a.id === action.next_item_interaction_action_id);
				if (targetAction) {
					newEdges.push({
						id: `${createItemInteractionActionNodeId(action)}-next-${createItemInteractionActionNodeId(targetAction)}`,
						source: createItemInteractionActionNodeId(action),
						sourceHandle: 'next',
						target: createItemInteractionActionNodeId(targetAction),
						targetHandle: 'target',
...
>
	...
	{#if selectedAction}
		<ItemInteractionActionNodePanel
			action={selectedAction}
			hasParent={selectedActionHasParent}
			{itemInteractionId}
		/>
	{:else}
		<ItemInteractionActionPanel {interaction} {onlayout} {itemInteractionId} />
	{/if}
</SvelteFlow>
