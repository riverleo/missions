<script lang="ts">
	...
	const nodeTypes = {
		action: CharacterInteractionActionNode,
	};
	...
	function isValidConnection(connection: Connection | Edge) {
		...
		if (sourceNode.type === 'action' && targetNode.type === 'action') {
			...
			const sourceActionId = parseCharacterInteractionActionNodeId(connection.source);
			const sourceAction = actions.find((a: CharacterInteractionAction) => a.id === sourceActionId);
			if (sourceAction && sourceAction.next_character_interaction_action_id) {
				return false;
			}
	...
	const onconnectend: OnConnectEnd = async (event, connectionState) => {
		...
		const fromActionId = parseCharacterInteractionActionNodeId(sourceNode.id);
		const fromHandleId = connectionState.fromHandle?.id;
		const fromAction = actions.find((a: CharacterInteractionAction) => a.id === fromActionId);
		...
		if (fromHandleId === 'next' && fromAction.next_character_interaction_action_id) return;
		...
		try {
			...
			if (newAction) {
				nodes = nodes.map((n) =>
					n.id === createCharacterInteractionActionNodeId(newAction) ? { ...n, position } : n
				);
			}
		} catch (error) {
			skipConvertEffect = false;
	...
	function convertToNodesAndEdges() {
		...
		// 액션 노드
		actions.forEach((action: CharacterInteractionAction, index: number) => {
			...
			const id = createCharacterInteractionActionNodeId(action);
			newNodes.push({
				id,
				type: 'action',
			...
			if (action.next_character_interaction_action_id) {
				...
				if (targetAction) {
					newEdges.push({
						...
						source: createCharacterInteractionActionNodeId(action),
						sourceHandle: 'next',
						target: createCharacterInteractionActionNodeId(targetAction),
						targetHandle: 'target',
...
>
	...
	{#if selectedAction}
		<CharacterInteractionActionNodePanel
			action={selectedAction}
			hasParent={selectedActionHasParent}
			{characterInteractionId}
		/>
	{:else}
		<CharacterInteractionActionPanel {interaction} {onlayout} {characterInteractionId} />
	{/if}
</SvelteFlow>
