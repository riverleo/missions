<script lang="ts">
	...
	const currentPriorityId = $derived(page.params.priorityId);
		...
		<ButtonGroup>
			...
			<ButtonGroup>
				<Tooltip>
					<TooltipTrigger>
						{#snippet child({ props })}
							<Button {...props} variant="outline" size="icon" disabled>
								<IconPlus class="size-4" />
							</Button>
						{/snippet}
					</TooltipTrigger>
					<TooltipContent>새로운 우선도 생성</TooltipContent>
				</Tooltip>
				<Tooltip>
					<TooltipTrigger>
						{#snippet child({ props })}
							<Button {...props} variant="outline" size="icon" disabled={!currentPriorityId}>
								<IconEditCircle class="size-4" />
							</Button>
						{/snippet}
					</TooltipTrigger>
					<TooltipContent>우선도 수정</TooltipContent>
				</Tooltip>
			</ButtonGroup>
		</ButtonGroup>
		<ButtonGroup>
			<Tooltip>
				...
				<TooltipContent>우선도 삭제</TooltipContent>
			</Tooltip>
		</ButtonGroup>
	</ButtonGroup>
