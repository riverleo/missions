...
{#snippet equipmentSlot(slot: EquipmentSlot)}
	...
	>
		<div
			class="flex h-full w-full flex-col items-center justify-center gap-1 {equipment ? 'cursor-grab active:cursor-grabbing' : ''}"
			draggable={!!equipment}
		...
		>
			...
			{#if equipment}
				<div class="mt-1 text-center">
					<div class="text-xs font-semibold leading-tight">{equipment.name}</div>
					<Badge
						variant="secondary"
						class="mt-1 h-4 text-[10px] {rarityColors[equipment.rarity]}"
					>
						{equipment.rarity}
					</Badge>
				</div>
				...
				<div class="grid gap-4 lg:grid-cols-2">
					...
					<div class="space-y-2">
						...
						<div class="grid grid-cols-5 gap-2">
							...
							<!-- Character Avatar -->
							<div
								class="flex items-center justify-center rounded-lg border-2 border-dashed border-primary/30 bg-primary/5 p-2 min-h-[90px]"
							>
