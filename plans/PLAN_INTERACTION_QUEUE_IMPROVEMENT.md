...
- [x] `ready` 상태에서 첫 `InteractionTargetId`를 `currentInteractionTargetId`에 설정하고 상태를 `action-running`으로 전환한다.
...
- [ ] `tick-action-system-item-pick.ts` 파일을 추가한다.
- [ ] 아이템 줍기 완료 시점에만 `world_items.world_character_id`와 `heldItemIds`를 동기화한다.
- [ ] 아이템 엔티티 월드 제거 시점을 `item_pick` 액션 실행 성공 시점으로 제한한다.
- [ ] `item_pick`은 이동 도달 조건(가까움 또는 path 빈 배열) 충족 전까지 실행 시작하지 않는다.
- [ ] 이동 도달 조건이 충족되는 틱에 `currentInteractionTargetStartedAtTick`를 기록한다.
- [ ] `tick-action-system-item-pick` 단위 테스트를 추가한다.

### 3) 실행 파이프라인 연결

- [ ] 상호작용 액션 타입에 따라 시스템 액션 tick을 분기 호출하는 엔트리 포인트를 설계한다.
- [ ] 큐 상태(`ready/action-running/action-completed/completed`)와 각 틱 액션 실행 상태가 충돌하지 않도록 순서를 명시한다.
- [ ] 기존 `tick-enqueue-interaction-queue`, `tick-dequeue-interaction` 테스트를 회귀 검증한다.
- [x] `tick-dequeue-interaction`의 상태 전이 규칙(`ready -> action-running`, `action-completed -> action-running|completed`)을 테스트로 고정한다.

### 4) InteractionQueue 타입 개선

- [x] `poppedAtTick`를 제거하고 `currentInteractionTargetStartedAtTick`로 대체한다.
- [x] `currentInteractionTargetId?: InteractionTargetId` 필드를 추가한다.
- [x] `currentInteractionTargetStartedAtTick?: number` 필드를 추가한다.
- [x] 상태 타입을 `enqueuing | ready | action-running | action-completed | completed`로 세분화한다.
